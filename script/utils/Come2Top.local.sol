...
contract Come2Top {
    ...
    address public immutable BLOCK_HASHER;
    ...
    */
    function _getRandomSeed(uint256 startBlock) private view returns (uint256) {
        ...
        while (blockHashes[ZERO] == ZERO) {
            blockHashes[index] = uint256(
                IBlockHasher(BLOCK_HASHER).blockHash(startBlock - b)
            );
...
interface IBlockHasher {
    function blockHash(uint256 blockNo) external view returns (bytes32);
}
